...
(defun hva-q-to-m3s (q-m3h)
  (/ q-m3h 3600.0))

;; Kesit alanı: A = Q/V  (Q m³/s, V m/s -> A m²)
(defun hva-area-from-qv (q-m3s v)
  (/ q-m3s v))

;; Hız: V = Q/A
(defun hva-velocity-from-qa (q-m3s a)
  (/ q-m3s a))

;; Huebscher eşdeğer çap (mm). De = 1.30*(a*b)^0.625 / (a+b)^0.250
(defun hva-equiv-diameter (a b)
  (/ (* 1.30 (expt (* a b) 0.625))
     (expt (+ a b) 0.250)))
...
(defun hva-rect-from-area-ar (a-m2 ar / a-mm2 b a)
...
(defun hva-size-constant-velocity (q-m3h v ar / q a)
  (setq q (hva-q-to-m3s q-m3h))
  (setq a (hva-area-from-qv q v))
  (hva-rect-from-area-ar a ar))
...
(setq *hva-friction-k* 0.022243)
(setq *hva-friction-q-exp* 1.852)
(setq *hva-friction-d-exp* 4.973)

(defun hva-friction-rate (q-m3s de-mm / de-m)
  (setq de-m (/ de-mm 1000.0))
  (/ (* *hva-friction-k* (expt q-m3s *hva-friction-q-exp*))
     (expt de-m *hva-friction-d-exp*)))

;; Eşit sürtünme: hedef Pa/m'yi sağlayan eşdeğer çapı bul, sonra en-boy oranıyla a,b.
(defun hva-size-equal-friction (q-m3h target-pam ar / q de-m a-m2)
  (setq q (hva-q-to-m3s q-m3h))
  (setq de-m (expt (/ (* *hva-friction-k* (expt q *hva-friction-q-exp*)) target-pam)
                   (/ 1.0 *hva-friction-d-exp*)))
  (setq a-m2 (* 3.14159265 (/ (* de-m de-m) 4.0)))
  (hva-rect-from-area-ar a-m2 ar))
...
(defun hva-size-full (method q-m3h target ar
                      / raw a-raw b-raw a b q a-m2 v-real pam-real de)
  (cond
    ((= method "CV") (setq raw (hva-size-constant-velocity q-m3h target ar)))
    ((= method "EF") (setq raw (hva-size-equal-friction q-m3h target ar)))
    (t (setq raw nil)))
  (if (null raw)
    nil
    (progn
      (setq a-raw (car raw) b-raw (cadr raw))
      (setq a (hva-round-up-standard a-raw))
      (setq b (hva-round-up-standard b-raw))
      (setq q (hva-q-to-m3s q-m3h))
      (setq a-m2 (/ (* a b) 1000000.0))
      (setq v-real (hva-velocity-from-qa q a-m2))
      (setq de (hva-equiv-diameter a b))
      (setq pam-real (hva-friction-rate q de))
