...
(hk-assert-close "3600 m3h -> 1 m3s" 1.0 (hva-q-to-m3s 3600.0))
(hk-assert-close "A = Q/V" 0.2 (hva-area-from-qv 1.0 5.0))
(hk-assert-close "De 400x300" 377.709 (hva-equiv-diameter 400.0 300.0))
(hk-assert-close "V = Q/A" 5.0 (hva-velocity-from-qa 1.0 0.2))

(setq dim (hva-rect-from-area-ar 0.2 1.5))
(hk-assert-close "rect a (ar=1.5)" 547.7226 (car dim))
(hk-assert-close "rect b (ar=1.5)" 365.1484 (cadr dim))

(setq r (hva-size-constant-velocity 3600.0 5.0 1.5))
(hk-assert-close "cv a" 547.7226 (car r))
(hk-assert-close "cv b" 365.1484 (cadr r))

(setq pam (hva-friction-rate 0.5 300.0))
(hk-assert-true "friction pozitif" (> pam 0.0))
(hk-assert-true "friction makul aralik (0.1-20 Pa/m)"
                (and (> pam 0.1) (< pam 20.0)))

(setq r2 (hva-size-equal-friction 3600.0 1.0 1.5))
...
(setq res (hva-size-full "CV" 3600.0 5.0 1.5))
